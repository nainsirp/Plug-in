...
const DAW = () => {
  ...
  const dawProducts = products.filter(p => p.category === 'daw');
  const filteredProducts = dawProducts.filter(p => p.os.toLowerCase() === selectedOS);

  return (
    <div style={{ paddingTop: '40px', minHeight: '80vh' }} className="container">
      <h1 className="section-title text-glow">DAW Software</h1>
      ...
      {selectedOS === 'mac' && (
        <div style={{
          ...
          <div>
            ...
            Mac installation can sometimes be complex. Download and install at your own risk. There is <strong>STRICTLY NO REFUND</strong> for Mac users once purchased, but the software is guaranteed to be in full working condition.
          </div>
        </div>
      )}
